...
class ProjectRepo(FileRepository):
    ...
    def workdir(self) -> Path:
        # print(Path(self.git_repo.workdir),'-----------path------------')
        return Path(self.git_repo.workdir)

    @property
    def srcs(self) -> FileRepository:
        # print(self._git_repo.new_file_repository(self._srcs_path),'-----------path2------------')
    ...
    def code_files_exists(self) -> bool:
        ...
        src_workdir = git_workdir / git_workdir.name   #代码文件存放目录
        Task.get_src_path(src_workdir)
        # print(git_workdir,'-----------git_workdir------------')
        # print(src_workdir,'-----------src_workdir------------')
