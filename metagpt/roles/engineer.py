...
# from metagpt.pyscripttask import Task
...
class Engineer(Role):
    ...
    constraints: str = (
        ...
        "If TCP communication is involved, the target IP is 127.0.0.1 and the port number is 30003, and pay attention to the content to be sent"
        "do not send unnecessary content such as 'hello'"
    )
    n_borg: int = 1
    ...
    async def _act_sp_with_cr(self, review=False) -> Set[str]:
        ...
        for todo in self.code_todos:
            ...
            # print(coding_context.filename,'-----main')
            # Code review
            if review:
            ...
            # Task.get_enterfile(coding_context.filename)
            await self.project_repo.srcs.save(
                filename=coding_context.filename,#编码文件 game.py main.py....
